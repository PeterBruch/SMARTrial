...
```{r}
tibble("Subcohort"=c("Chemotherapy cohort", "CLL cohort treated with \nVenetoclax or Ibrutinib"), "n"=c(46,18)) %>% 
...
ggsave("../../docs/Figure_S4.pdf", plot = p$gtable, height = 15, width =15)
```
...
plotTab <- group_by(df_p_drugAUC, Pathway, response) %>% 
  summarise(AUC_pathway_response = mean(AUC)) %>% 
  #difference R vs PD
  group_by(Pathway) %>%
  summarise(diff_RvsPD = AUC_pathway_response[response == "R"] - AUC_pathway_response[response == "PD"], 
            direction = ifelse(diff_RvsPD < 0,"decreased","increased")) %>% ungroup() %>%
...
waterfall_plot_Rvs_PD<-ggplot(plotTab, aes(x=Pathway, y=diff_RvsPD, fill=direction)) +
  ...
  theme_figures_x_angle_wo_legend+
  ggtitle("\u0394 = viability (R) – viability (PD)")


waterfall_plot_Rvs_PD
...
ggsave(path=(paste0("../../docs/")), filename = "Figure_S7.pdf", device = cairo_pdf)
```
...
wrap_elements(plot_3C) + wrap_elements(Fig3D) + wrap_elements(plot_3E) + wrap_elements(plot_3F) +
...
ggsave( path=(paste0(filepath,"docs")), filename = "Figure_S9.svg")
...
figure_CLL<-
  ...
  plot_annotation(tag_levels ="A") & theme(title = element_text(size=18))

figure_CLL

ggsave(path=(paste0(filepath,"docs")), filename = "Figure_S10.pdf")
...
```{r, fig.width=14, fig.height=19.8}
tp<-theme(plot.tag = element_text(size=22, face="bold"))

wrap_elements(ED_Fig_1A)+tp+
  ggplotify::as.ggplot(ED_Fig_1B)+tp+
  plot_layout(ncol=1, heights = c(1,3))+
  plot_annotation(tag_levels = "A")


ggsave( path=(paste0(filepath,"docs")), filename = "Figure_ED1.pdf")
```
...
ggsave( path=(paste0(filepath,"docs")), filename = "Figure_S6.pdf")
  
#ggsave(width = 16, height =24, path=(paste0(filepath,"docs")), filename = "Figure_S2.pdf")
...
ggsave( path=(paste0(filepath,"docs/")), filename = "Figure_S8.pdf")
```
