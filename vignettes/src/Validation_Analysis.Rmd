...
It includes analyses of Junyan Lu, iwth extensions and modifications by Peter-Martin Bruch.
...
pheatmap::pheatmap(corMat, color = colorRampPalette(c("#003DA5", "white", "#A6093D"))(100), 
         filename=paste0("../../docs/Figure_S3.pdf"),
...
ggsave("../../docs/HC_heatmap_Validation.pdf", plot = p$gtable, height = 8, width =15)
...
```{r Figure_S13, fig.height=25, fig.width=15, warning=FALSE, message=FALSE, fig.path="../../docs/", dev=c("png","pdf")}
pList <- lapply(seq(nrow(resTab.sig)), function(i) {
    rec <- resTab.sig[i,]
    leg <- NULL
...
ggsave("../../docs/Figure_S11.pdf", height = 12, width = 20)
```
...
ggsave("../../docs/Figure_S12.pdf", height = 20, width =10)
```
